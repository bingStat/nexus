...
$sshCmd = "ssh -o StrictHostKeyChecking=no root@192.168.31.88 `"cat << 'EOF' > /root/modem_proxy.py`n" + $pyCode + "`nEOF`n docker rm -f modem_proxy || true && docker run -d --name modem_proxy --restart always --network host -v /root/modem_proxy.py:/modem_proxy.py docker.m.daocloud.io/library/python:3.11-slim python3 /modem_proxy.py`""
