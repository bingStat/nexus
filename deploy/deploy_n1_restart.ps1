...
$sshCmd = "ssh -o StrictHostKeyChecking=no root@${n1IP} '/usr/bin/cloudflared service uninstall; /usr/bin/cloudflared service install $token; /etc/init.d/cloudflared start; /etc/init.d/cloudflared status'"
