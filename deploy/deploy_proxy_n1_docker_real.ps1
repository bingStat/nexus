...
$sshCmd = "ssh -o StrictHostKeyChecking=no root@192.168.31.88 'echo $encoded | base64 -d > /root/modem_proxy.py && docker rm -f modem_proxy || true && docker run -d --name modem_proxy --restart always --network host -v /root/modem_proxy.py:/modem_proxy.py docker.m.daocloud.io/library/python:3.11-slim python3 /modem_proxy.py'"
