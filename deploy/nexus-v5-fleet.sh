...
install_root_worker() {
  ...
  ssh_cmd "$target" "curl -fsSL https://raw.githubusercontent.com/bingStat/nexus/$REF/deploy/nexus-v5.sh | NEXUS_ROLE=worker NEXUS_DEVICE_ID=$device NEXUS_REF=$REF NEXUS_TOKEN_SOURCE=$remote_token NEXUS_RETIRE_V3=0 sh; rm -f $remote_token"
}
