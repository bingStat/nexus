...
# Step 1: TC downloads from Victus HTTP server
Write-Host "=== Step 1: TC pulls cloudflared from Victus:19876 ==="
$out = Send-NexusCmd -target "thinkcenter" -cmd "curl -f http://${victusIP}:19876/cloudflared-arm64 -o /tmp/cloudflared && chmod +x /tmp/cloudflared && stat -c %s /tmp/cloudflared" -waitSec 50

if ($out -notlike "*TIMEOUT*" -and $out -match "\d{5,}") {
    Write-Host "=== Download OK, size=$($Matches[0]) bytes ==="
} else {
...
$out2 = Send-NexusCmd -target "thinkcenter" -cmd "scp -o StrictHostKeyChecking=no /tmp/cloudflared root@${n1IP}:/tmp/cloudflared && echo SCP_OK" -waitSec 30
