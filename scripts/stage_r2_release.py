...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage the Nexus website R2 release tree")
    parser.add_argument("output", type=Path)
    args = parser.parse_args()
    release = stage(args.output.resolve())
    print(f"staged {len(release['files']) + 1} R2 website objects in {args.output}")
