...
$payload = @{ id=$id; command="curl -f -L https://rca-enable-lesson-assignment.trycloudflare.com/cloudflared-arm64 -o ~/cloudflared && stat -c %s ~/cloudflared"; target_device="thinkcenter"; status="pending"; timeout_ms=120000 } | ConvertTo-Json -Compress
