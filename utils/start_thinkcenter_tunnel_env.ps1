...
$payload = @{ id=$id; command="su - bing -c 'unset WAYLAND_DISPLAY XDG_SESSION_TYPE DISPLAY; nohup /snap/bin/code tunnel --name thinkcenter --accept-server-license-terms > /tmp/code_tunnel.log 2>&1 &'"; target_device="thinkcenter"; status="pending"; timeout_ms=30000 } | ConvertTo-Json -Compress
