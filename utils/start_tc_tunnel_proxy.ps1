...
$payload = @{ id=$id; command="killall code-tunnel; su - bing -c 'export https_proxy=http://192.168.31.88:7890 http_proxy=http://192.168.31.88:7890; nohup /snap/code/current/usr/share/code/bin/code-tunnel tunnel --name thinkcenter --accept-server-license-terms > /tmp/code_tunnel.log 2>&1 &'"; target_device="thinkcenter"; status="pending"; timeout_ms=30000 } | ConvertTo-Json -Compress
...
Write-Host "Sent cmd $id to thinkcenter"
for ($i = 0; $i -lt 15; $i++) {
    Start-Sleep 2
