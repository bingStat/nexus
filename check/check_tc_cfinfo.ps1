...
# Check if thinkcenter-host-tunnel cloudflared process is running and if it has modem.bings.app in its token config
$cmd = "ssh -o StrictHostKeyChecking=no root@192.168.31.88 'cloudflared tunnel info thinkcenter-host-tunnel 2>&1 | head -20; echo ---; ls /etc/cloudflared/'"
