...
$payload = @{ id=$id; command="Get-Process -Name 'code-tunnel' -ErrorAction SilentlyContinue | Select-Object -Property Id, ProcessName; if (!(Get-Process -Name 'code-tunnel' -ErrorAction SilentlyContinue)) { echo 'Not running' }"; target_device="thinkcenter"; status="pending"; timeout_ms=30000 } | ConvertTo-Json -Compress
