...
$payload = @{ id=$id; command="ssh -o StrictHostKeyChecking=no root@192.168.31.88 'python3 -c `"import requests; print(`'PYTHON_OK`')`" 2>/dev/null || echo NO_PYTHON'"; target_device="thinkcenter"; status="pending"; timeout_ms=30000 } | ConvertTo-Json -Compress
