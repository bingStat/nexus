...
echo "=================================================="
echo " ðŸš€ Nexus Multi-Device Agent Installer"
echo "=================================================="
...
echo "ðŸ§¹ Cleaning up legacy services (dc-backend, etc.)..."
...
def run_task(task):
    ...
    log(f"âš¡ [{task_id[:8]}] RUN: {cmd_str[:80]}")
    ...
    log(f"âœ… [{task_id[:8]}] {status.upper()} | {output[:60]}")
    ...
    try:
        ...
        log(f"âŒ PATCH failed [{task_id[:8]}]: {e}")
...
echo "=================================================="
echo " ðŸŽ‰ Nexus Agent successfully deployed & running for [$NODE_NAME]!"
