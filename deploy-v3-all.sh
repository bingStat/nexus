...
printf '%s\n' '[1/7] Deploy Oracle registry, EU broker, agent and MCP'
run_ssh "$ORACLE_HOST" "curl -fsSL '$SOURCE_BASE/install-v3.sh' -o /tmp/install-v3.sh && sudo env NEXUS_SOURCE_BASE='$SOURCE_BASE' NEXUS_V3_REGISTRY_URL=https://nexus-global-api.bings.app NEXUS_V3_BROKER_URL=http://127.0.0.1:18102 sh /tmp/install-v3.sh oracle"
...
printf '%s\n' '[2/7] Deploy ThinkCenter CN broker and agent'
run_ssh "$THINKCENTER_HOST" "curl -fsSL '$SOURCE_BASE/install-v3.sh' -o /tmp/install-v3.sh && env NEXUS_SOURCE_BASE='$SOURCE_BASE' NEXUS_V3_REGISTRY_URL=https://nexus-global-api.bings.app NEXUS_V3_BROKER_URL=http://127.0.0.1:18120 sh /tmp/install-v3.sh thinkcenter"

printf '%s\n' '[3/7] Refresh N1 bootstrap mirror'
run_ssh "$THINKCENTER_HOST" "mkdir -p /opt/nexus-bootstrap && for f in install-openwrt-v3.sh openwrt_v3_agent.sh openwrt_ed25519_signer.rb; do curl -fsSL '$SOURCE_BASE/'\"\$f\" -o /opt/nexus-bootstrap/\"\$f\"; done"

printf '%s\n' '[4/7] Deploy N1 agent through ThinkCenter jump host'
...
run_ssh "$ORACLE_HOST" "sudo python3 /opt/nexus-v3/scripts/approve_devices.py oracle thinkcenter n1 2>/dev/null || true"
...
run_ssh "$ORACLE_HOST" "sudo python3 /opt/nexus-v3/scripts/verify_v3.py oracle thinkcenter n1"
