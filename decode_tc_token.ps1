...
# Check the thinkcenter-host-tunnel token to find its tunnel ID
$cmd = "head -c 500 /etc/cloudflared/token | cut -d. -f2 | base64 -d 2>/dev/null || head -c 500 /etc/cloudflared/token"
