...
本报告是 Nexus v2.5-regional 的完整软件与服务级结项报告。相关长期知识已同步到：

1. `README.md`：最终生产基线与文档导航；
2. `任务图谱.md`：已关闭 P0、维护期验证与后续 P1；
3. `成果归档.md`：发布物、提交、Tag 与验收证据；
4. `技术细节演进.md`：Global API、区域 Broker、execution ledger 与 Browser Bridge 设计；
5. `文献与参考.md`：最终 API、运行组件和证据路径；
6. `日志.jsonl`：机器可读结项事件；
7. `经验.md`：Windows worker、Token、时区和 Claude 完成检测排障经验；
8. `记忆.md`：直接目标调度、真实回执和浏览器安全边界；
9. `评估与实验分析.md`：最终测试矩阵与结果。

整机重启演练因需要明确维护窗口，作为已记录的运维验证项，不被描述为已完成。
