...
$bashScript = @"
cat << 'PYTHON_EOF' > /tmp/modem_proxy.py
$pyCode
PYTHON_EOF
scp -o StrictHostKeyChecking=no /tmp/modem_proxy.py root@192.168.31.88:/root/modem_proxy.py
ssh -o StrictHostKeyChecking=no root@192.168.31.88 'docker restart modem_proxy'
"@

$encodedBash = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($bashScript))

$finalCmd = "echo $encodedBash | base64 -d | bash"

$payload = @{ id=$id; command=$finalCmd; target_device="thinkcenter"; status="pending"; timeout_ms=30000 } | ConvertTo-Json -Compress
