...
class NexusAPI:
    ...
    def aliases(config: dict[str, Any]) -> list[str]:
        values = {
            str(config["device_id"]), str(config["device_name"]),
            socket.gethostname(), socket.gethostname().lower(),
            *[str(item) for item in config.get("aliases", [])],
        }
        return sorted(value for value in values if value and all(ch.isalnum() or ch in "-_ " for ch in value))
    ...
    while True:
        try:
            ...
            if not capacity_available:
                ...
                continue
            task = api.broker_claim(config) if api.broker_urls else api.claim(config)
            error_backoff = config["poll_seconds"]
            if task:
